# TypeFlow – тренажёр слепой печати в стиле Minecraft

# TypeFlow – тренажёр слепой печати в стиле Minecraft

![Version](https://img.shields.io/badge/version-v0.1--alpha-blue)
![Python](https://img.shields.io/badge/python-3.12-green)
![License](https://img.shields.io/badge/license-MIT-yellow)

## Скриншоты
![Главное окно](screenshots/main.png)
![Выбор уроков](screenshots/lessons.png)
![Статистика](screenshots/stats_history.png) 
![Звук](screenshots/sounds.png) 

## Демо
https://asciinema.org/a/J9DoPrYmMKnX2qYQ

## Возможности
- Два режима: Minecraft / Terminal
- Система пользователей с сохранением прогресса
- 30+ уроков: от базовых позиций до кода (Bash, Python, Git, K8s)
- Алмазы и достижения
- Выбор языка (RU/EN)

## Установка и запуск
```bash
git clone https://github.com/Alex305305/typeflow.git
cd typeflow
pip install -r requirements.txt
python main.py